    ...
    for i in range(rounds):
        ...
        if i == 0:
            ...
            retailer_position.append(round(sum(orders_from_retailer[-3:]) 
                                           + sum(wholesaler_backorder[-4:]) - retailer_backorder[i]))
            wholesaler_position.append(round(sum(orders_from_wholesaler[-3:]) 
                                             + sum(distributor_backorder[-4:]) - wholesaler_backorder[i]))
            distributor_position.append(round(sum(orders_from_distributor[-3:]) 
                                              + sum(manufacturer_backorder[-4:]) - distributor_backorder[i]))
            manufacturer_position.append(round(sum(orders_from_manufacturer[-3:]) 
                                               + sum(orders_from_distributor[-4:]) - manufacturer_backorder[i]))
        ...
        print('Inventory: ', distributor_inventory[i], 'BEER')
        print('Balance: ', distributor_balance[i], 'ETH')
        if i != 0:
            print('Last order: ', orders_from_distributor[i-1])
        print('Orders from client: ', orders_from_wholesaler[i], 'BEER')
        print('Backorder due to client: ', distributor_backorder[i], 'BEER')
        print('Backorder due from supplier: ', manufacturer_backorder[i])
        var = input("Enter an amount to order: ")
        print()
        
        orders_from_distributor.append(max(0, round(int(var))))
...
if __name__ == '__main__':
    ...
    df = the_beer_game(250000, 12, 0.002, 4, 10)
